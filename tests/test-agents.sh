...
out="$(render_agent implementer codex.toml)"
...
out="$(render_agent final-reviewer codex.toml)"
...
out="$(render_agent plan-author codex.toml)"
assert_contains "$out" 'model = "gpt-5.6-terra"' "plan-author/codex: think は terra"

out="$(render_agent re-reviewer codex.toml)"
...
out="$(render_agent task-reviewer codex.toml)"
assert_contains "$out" 'approval_policy = "on-request"' \
...
impl_toml="$(mktemp)"
render_agent implementer codex.toml > "$impl_toml"
...
read_toml="$(mktemp)"
render_agent task-reviewer codex.toml > "$read_toml"
...
# Claude 版。
out="$(render_agent implementer claude.md)"
...
out="$(render_agent final-reviewer claude.md)"
...
# opencode 版。
out="$(render_agent implementer opencode.md)"
...
out="$(render_agent final-reviewer opencode.md)"
...
# manifest に role を足したのに定義が展開できないと、Paseo MCP でその role を起動できない。
...
# レビュー役のプロンプトは herdr に触れない。
for a in task-reviewer re-reviewer final-reviewer; do
  body="$(render_template "agent-defs/prompts/$a.md" claude)"
