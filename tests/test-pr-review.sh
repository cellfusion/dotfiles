...
assert_contains "$skill" '.local/state/pr-review' "pr-review: 成果物を worktree の外へ保存する"
assert_not_contains "$skill" "_cellfusion/reviews" "pr-review: 成果物を呼び出し元 checkout に置かない"
assert_not_contains "$skill" "cellfusion-workdir" "pr-review: cellfusion-workdir を呼ばない"
