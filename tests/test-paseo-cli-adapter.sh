...
case "$1" in
  run)
    [ "$5" = "pi" ] && [ "$7" = "openai-codex/sample-light" ] && [ "${10}" = "--workspace" ] && [ "${11}" = "workspace-1" ] || exit 3
    printf '%s\n' '{"agentId":"agent-1"}'
    ;;
  ...
  provider)
    [ "$2" = "ls" ] || exit 3
    printf '%s\n' '[{"provider":"pi","status":"available","enabled":"Enabled","defaultMode":"default","modes":""}]'
    ;;
  inspect) printf '%s\n' '{"Id":"agent-1","Status":"idle","CreatedAt":"2026-09-20T08:00:00.000Z","UpdatedAt":"2026-09-20T08:02:00.000Z","LastUsage":{"InputTokens":100,"CachedTokens":80,"OutputTokens":20,"CostUsd":0.12}}' ;;
  *) exit 2 ;;
esac
SH
chmod +x "$fake"
...
providers="$(PASEO_CLI="$fake" "$adapter" list-providers)"
assert_eq "$providers" '{"providers":[{"id":"pi","available":true,"modeIds":["auto"]}]}' 'CLI adapter aliases Pi default mode for strict launch'
