...
self-review を通したら、承認 gate を出す**前**にファイルを別の端末で開く。開くかどうかはユーザーに聞かない。

環境で経路が分かれる。上から順に判定し、最初に当たった経路だけを実行する。

| 条件 | 経路 |
|---|---|
| `$PASEO_AGENT_ID` が非空 | 下の「Paseo の端末で開く」 |
...
```bash
term=$(paseo terminal create --cwd "$PWD" --name "spec-preview" --json | jq -r '.id')
paseo terminal send-keys "$term" 'glow -p "<ファイルの絶対パス>"; exit' Enter
```

- 端末は relay 越しに外部からも見え、キー入力も送れる
- `--name` は brainstorming なら `spec-preview`、writing-plans なら `plan-preview` にする。
  同じ名前の端末が増えないようにする
- **末尾の `; exit` を省かない。** これが無いと `glow` を終了しても shell が残り、
  空の端末が残り続ける
- `send-keys` に渡す文字列は**シングルクォートで囲む**。ダブルクォートで囲むと
  自分の側で展開してしまう
- パスは絶対パスで渡し、ダブルクォートで囲む
- `paseo terminal create` は workspace を指定するオプションを持たない。`--cwd` で決まる

`term` が空か `null` なら、その旨を 1 行伝えてプレビューなしで承認 gate へ進む。
失敗を理由に作業を止めない。

`glow` が PATH に無いときも、1 行伝えて承認 gate へ進む。別のコマンドに置き換えない。

**自分から端末を閉じるコマンドを呼んではならない。** ユーザーが読んでいる最中に閉じると、
読めなくなる。`; exit` により、`glow` を終了した時点で端末が消える。

`glow` は編集できない。Paseo の経路では、ユーザーがその場で本文を直せない。
修正は承認 gate の「その他」の自由入力で受け取る。
