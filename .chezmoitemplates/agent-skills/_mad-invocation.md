...
役割エージェントが読める範囲は、Paseo が決める cwd に縛られる。**`--arg` で渡すパスは、その
cwd の下にあること。** cwd の決まり方は 3 通りある。

- `--workspace` で動くノードは、その workspace のディレクトリを cwd にする。`implement` の
  実装とレビューのノード、`spike` の実装のノードが該当する。読める範囲は作った worktree に
  なる。
- 素のシェルから `mad-run` を実行した場合、`--cwd` に渡した値が cwd になる。`mad-run` は
  レシピを打った場所をそのまま渡す。
- Paseo のエージェントの中から `mad-run` を実行した場合、`--cwd` は無視される。環境変数
  `PASEO_AGENT_ID` があると、Paseo の daemon は呼び出し元エージェントの cwd を役割
  エージェントに使う。

**Paseo のエージェントの中では、`mad-run` を呼び出し元エージェントの cwd で実行すること。**
別のリポジトリやサブディレクトリで実行すると、役割エージェントは呼び出し元の cwd を基点に
するので、`--arg` で渡した相対パスを解決できずにノードが失敗する。
