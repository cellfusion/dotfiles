...
|---|---|---|---|
| `research` | 観点別の調査役を並列に起動する。 | 調査役全件の `state.json` と成果物を親が確認する。 | 親の許可後だけ統合役へ調査成果物の絶対パスを渡す。失敗なら統合を停止する。 |
...
| `refine` | 批評観点ごとの批評役を並列に起動する。 | 親が改稿物、批評、各子の状態を確認する。 | 親が確認した批評成果物の絶対パスだけを改稿役へ渡す。失敗なら改稿と次ラウンドを停止する。 |
